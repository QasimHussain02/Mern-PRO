...
const FinishRideModal = ({ rideRequest, onComplete }) => {
  return (
    <motion.div
    ...
    >
      ...
      {/* rider row — amber accent to match the image */}
      <div className="mt-3 rounded-2xl bg-amber-400 px-3 py-2.5 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <img
            src={rideRequest.riderAvatar}
            alt={rideRequest.riderName}
            className="h-10 w-10 rounded-full object-cover border-2 border-black/10"
          />
      ...
      <button
        onClick={onComplete}
        className="mt-3 w-full rounded-xl bg-emerald-500 px-3 py-2.5 text-sm font-semibold text-black transition-colors hover:bg-emerald-400 cursor-pointer"
      >
        Complete Ride
      </button>
    </motion.div>
